...
window.onload = event => {
  if (oldWindowLoad7) oldWindowLoad7.bind(window)(event);

  if (document.querySelector("title")?.innerText.startsWith("Lab assignment 7")) {
...
namespace Lab7 {
  ...
  export const step2 = () => {
    ...
    try {
      ...
      pageProgram = expandProg(env, pageTree);
  ...
  export const submitInput = () => {
    ...
    try {
      ...
      if (isValue(expr) && equalTypes(inferExprType(pageProgram, new Map, expr), inputType!)) {
        inputValue = expr;
      } else
    ...
    if (tree instanceof NumLeaf) {
      ...
      for (const [funcName, func] of tree.entries()) {
        ...
        const paramString =
          '(' +
          func.parameterArray
            .map(param => param.name + ": " + typeToString(param.type))
            .join(", ") +
          ')';
        const typeString = func.returnType == null ? "" : `: ${typeToString(func.returnType)}`;
  ...
  const allConstantNames = (tree: PreExpr3 | Stmt<PreExpr3> | Program<PreExpr3>): Set<string> => {
